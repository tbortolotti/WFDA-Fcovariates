model_comparison_corrected <- function(mod.fit, t.points, name_dir, t.idxs, data, curves, set.log=FALSE)
{
  library(fda)
  library(coda)
  library(devtools)
  ...
  fondo1 <- adjustcolor(pal[3], alpha=0.3)
  fondo2 <- adjustcolor(pal[4], alpha=0.3)
  
  # create directory to save plots
  dir.current <- getwd()
  new.dir <- paste0(dir.current,"/Results/",name_dir,"/ITA18_comparison_corrected")
  dir.create(new.dir)
  ...
  a0       <- ITA18.regressors$a
  b1       <- ITA18.regressors$b1
  b2       <- ITA18.regressors$b2
  c1       <- ITA18.regressors$c1
  c2       <- ITA18.regressors$c2
  c3       <- ITA18.regressors$c3
  k0       <- ITA18.regressors$k
  f1       <- ITA18.regressors$f1
  f2       <- ITA18.regressors$f2
  ...
  for(i in 1:length(t.idxs)) #i=1
  {
    ...
    for(j in 1:length(vs.vec)) #j=1
    {
      ...
      for(k in 1:length(sof.vec)) #k=1
      {
        ...
        plot(log10dJB.seq, SA, type='l',lwd=4, col=pal[3],
        ...
        lines(log10dJB.seq, SA.ITA18.corr, lty=3, lwd=4, col=pal[1])
        ...
        lines(log10dJB.seq, SA.ITA18.corr, lty=3, lwd=4, col=pal[5])
        ...
        #       col=c(pal[2], pal[1], pal[5],'gray80', 'gray60','gray40'), lty=c(1,6,3,0,0,0), pch=c(NA,NA,NA,16,16,16), cex=0.8)
        legend(0, 0.6, legend=c("Functional", "Corrected", TeX("$M_w= 4.8$"),TeX("$M_w= 5.8$"), TeX("$M_w= 7.0$")),
               col=c('black', 'black',pal[3], pal[1],pal[5]), lwd=c(3,3,0,0,0),lty=c(1,3,0,0,0), pch=c(NA,NA,16,16,16), cex=2.2)
