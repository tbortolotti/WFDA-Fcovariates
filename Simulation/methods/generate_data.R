                          ...
                          case.info)
{
  ...
  {
    ## CASE 2
    ## Curves that are very noisy in part of the domain
    Pi <- rbinom(n.sim, size=1, prob=perc.noise)
    Pi.logic <- ifelse(Pi==1, TRUE, FALSE)
    T_hp <- rep(right.bound, n.sim)
    ...
    for(j in 1:length(t.grid))
    {
      for(i in 1:n.sim)
      {
        if(T_hp[i]>=t.grid[j])
        {
          y.vn[j,i] <- y.vn[j,i] + rnorm(1,0,smooth.noise)
        } else {
          y.vn[j,i] <- y.vn[j,i] + rnorm(1,0,ext.noise)
        }
      }
    }
